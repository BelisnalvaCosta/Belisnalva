# 👋 Olá, eu sou a Belisnalva Costa

Desenvolvedora **Trainee/Júnior** com foco em **Java**, **Python** e **FullStack**, tecnologias Web. Formada em **Tecnologia da Informação pela Univesp (2025)**, atuo em projetos que envolvem lógica, POO, API[...]  
Comprometida com boas práticas, aprendizado contínuo e desenvolvimento de soluções eficientes.

---

## 🚀 Tecnologias e Competências
- **Linguagens:** Java, Python, JavaScript
- **Web:** HTML5, CSS3  
- **Banco de Dados:** MySQL, SQLite  
- **Ferramentas:** Git, GitHub, VS Code, Postman, Streamlit  
- **Conceitos:** POO, lógica de programação, estruturas de repetição e decisão, APIs básicas  
- **Projetos Práticos:** jogos interativos, sistemas web, aplicações preditivas, dashboards e classificação lógica  

---

## 📚 Projetos em Destaque

### 🔹 Autoconhecimento e valores essenciais
- **Quiz Interativo ((HTML/CSS/JS)**  
  https://belisnalvacosta.github.io/quiz-autoconhecimento/

### 🔹 Desenvolvimento Web & Aplicações Interativas
- **E-book Interativo (HTML/CSS/JS)**  
  https://github.com/BelisnalvaCosta/ebook-learning
    
- **English Trainer Pro - Inglês com os verbos regulares e irregulares**

  https://belisnalvacosta.github.io/english-trainer/
  
- **Sistema de Heróis – Lógica de XP e Fases**  
  https://github.com/BelisnalvaCosta/desafio-projeto-herois

### 🔹 Back-End & Dados
- **Controle Financeiro com Streamlit + SQLite**  
  https://github.com/BelisnalvaCosta/controle_financeiro_streamlit_sqlite

### 🔹 Planejamento & Aprendizado
- **Plano de Estudo – Mentoria Tech**  
  https://github.com/BelisnalvaCosta/Mentoria_Plano_estudo

### 🔹 Projeto Integrador – Univesp
- **ConnectPharm – Sistema para Gestão Farmacêutica (Grupo PI)**  
  https://github.com/BelisnalvaCosta/ConnectPharm/blob/master/Estrutura

---

## 📊 Estatísticas do GitHub

<p align="center">
  <img src="https://github-readme-stats-sigma-five.vercel.app/api?username=BelisnalvaCosta&show_icons=true&theme=default" height="180em"/>
  <img src="https://github-readme-stats-sigma-five.vercel.app/api/top-langs/?username=BelisnalvaCosta&layout=compact&theme=default" height="180em"/>
</p>

---

## 🎓 Formação
**Bacharelado em Tecnologia da Informação – Univesp/SP**  
Conclusão: 2025

---

## 📘 Cursos Relevantes
- Java Backend (DIO.me)  
- Spread Java Developer  
- Code Girls  
- Python Essentials – IBM SkillsBuild  
- Introdução à Nuvem  
- Segurança da Informação  
- English4Tech (em andamento)

---

## 🎯 Objetivo Profissional
Atuar como **Desenvolvedora Trainee/Júnior** em Java ou Python, contribuindo para soluções tecnológicas eficientes e acessíveis.

---

## 🌐 Contatos
- **LinkedIn:** https://www.linkedin.com/in/belisnalva-costa-pereira/  
- **GitHub:** https://github.com/BelisnalvaCosta  
- **Portfólio:** https://belisnalvacosta.github.io/resume-Curriculo/
- **Portfólio atualizado - Projeto em Htmls e CSS:** https://belisnalvacosta.github.io/portfolio-atulizado/

---

## 📄 Licença
Este repositório utiliza **MIT License**.
